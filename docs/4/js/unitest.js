...
class TestCodeStr {
    ...
    */
    ...
    // コメント箇所内の)は無視したい。でも複雑すぎて実装が面倒。でも実装しないとコード箇所を正しく抽出できない。むしろそれ以外にもコード位置がずれる要因があるかも？
    // やはり手動でコード箇所を推測し抽出するのは危険。テストコードを必ず関数で渡すか、そうでないならコード表示を辞めたほうがいいか。
    // // 行コメント
    // /* */ 複数行コメント
    /*
    #isOneLineComment(str, i, r) {
        str.split('\n');
    }        
    #isMultiLineComment(str, i, r) {
