...
* HTML (DOM) [createHTMLDocument][],[createDocument][]でテスト用documentを生成すべきか？
    * Event
        * name
        * callbackFn
    * HTMLインライン（HTMLテキストで書かれた値）
        * tagName
        * attribute
            * key
            * value
            * inlineStyle(`element.style`)
                * HTMLテキストに書かれたインライン属性値`style=""`の内容（`getComputedStyle()`とは違う）
    * Node
        * type
            * elementNode
                * tagName
                * attribute  `hasKey`, `getAttribute('key')->valueは指定値と同じか`, 
                    * key
                    * value
                * style
                    * [getComputedStyle][]（現在適用中のCSSを確認する）
                    * [computedStyleMap][]（FireFox未実装）
            * textNode
                * value
            * 他
        * Input系value
        ...
        * 相対関係
            * 位置（親、子、兄、弟）
                * 存在
                    * 指定した要素P内に、指定した子要素Cが存在するか
                * 個数
                    * 指定した要素P内に、指定した子要素CがN個存在するか
* CSS [CSSオブジェクトモデル][] スタイルシートファイル内容をJSで参照・確認する (CSSOM)
    * [CSSStyleSheet][]
                    ...
                    * [styleMap][]
                        * [StylePropertyMap][]
                            * [StylePropertyMapReadOnly][]
                                * size,entries(),forEach(),get(),getAll(),has(),keys(),values()
                                    * どの[selectorText][]が持つ
                                    * どの[styleMap][]の
                                        * keyの数はいくつか
                                        * どの`key`があるか
...
[createDocument]:https://developer.mozilla.org/ja/docs/Web/API/DOMImplementation/createDocument
[createHTMLDocument]:https://developer.mozilla.org/ja/docs/Web/API/DOMImplementation/createHTMLDocument

[CSSオブジェクトモデル]:https://developer.mozilla.org/ja/docs/Web/API/CSS_Object_Model
[CSSStyleSheet]:https://developer.mozilla.org/ja/docs/Web/API/CSSStyleSheet
[CSSStyleSheet.cssRules]:https://developer.mozilla.org/ja/docs/Web/API/CSSStyleSheet/cssRules
[CSSRuleList]:https://developer.mozilla.org/ja/docs/Web/API/CSSRuleList
[CSSRule]:https://developer.mozilla.org/ja/docs/Web/API/CSSRule
[selectorText]:https://developer.mozilla.org/ja/docs/Web/API/CSSStyleRule/selectorText
[styleMap]:https://developer.mozilla.org/ja/docs/Web/API/CSSStyleRule/styleMap
[StylePropertyMap]:https://developer.mozilla.org/ja/docs/Web/API/StylePropertyMap
[StylePropertyMapReadOnly]:https://developer.mozilla.org/ja/docs/Web/API/StylePropertyMapReadOnly
[computedStyleMap]:https://developer.mozilla.org/ja/docs/Web/API/Element/computedStyleMap
...
## CSS 型付きオブジェクトモデル API [CSS Typed OM API][]

[CSS Typed OM API]:https://developer.mozilla.org/ja/docs/Web/API/CSS_Typed_OM_API
...
```javascript
unitest((a)=>{
    a.css((a)=>{// CSSファイルに対するテスト
        a.css('some.css', (a)=>{ // <html><head><link rel="stylesheet" href="some.css">
            a.rules.selector('h1:nth-child(2)').key('color').value('#FFFFFF');
        });
    });
    a.html((a)=>{// HTMLファイルまたは動的変更結果に対するテスト　documentが対象または親
        a.html(`main > p > button`, (a)=>{// buttonが対象または親
            a.ev('click').times(1); // クリックイベントを実行する。一回実行されたことを確認する（dispatchEvent(), spy）
            a.isElement; // node.nodeType = ELEMENT_NODE/TEXT_NODE
            a.notText;   // node.nodeType が TEXT_NODE でないこと
            a.tagName('button'); // 大文字小文字を問わず（tagNameは通常大文字だが現在HTMLは普通小文字で書く。齟齬をなくすため大文字小文字不問とする）
            a.attr('id', 'my-button-1'); // getAttribute()
            a.textContent = '実行ボタン'; // textContentが'実行ボタン'であること
            a.style.display = 'inline'; // button要素のCSSプロパティのうち`display`が`inline`であること
        });
    });
})
```
...
[nodeType]:https://developer.mozilla.org/ja/docs/Web/API/Node/nodeType

### テスト用document

　呼出元documentにHTML要素を直接挿入するとテスト内容ごとに要素が混入して競合したり不整合を起こしかねない。そこで一つのテスト毎に新しいdocumentを生成する。

```javascript
unitest((a)=>{
    a.cls(TestTargetClass, [], (a,testTarget)=>{// HTMLファイルまたは動的変更結果に対するテスト　documentが対象または親
        // 新しいdocument1で実行する（セレクタがない場合は新しいdocumentを生成する）
        // テスト対象におけるDOM操作APIを実行する
        testTarget.appendButtonElement();
        a.html(`main > p > button`, (a)=>{// buttonが対象または親。セレクタに該当する要素がなければテストエラー
            a.nodeType = 'element'; // element,attribute,text,cdata,processing,comment,document,doctype,docfrag,entry
            a.tagName = 'button'; // 大文字小文字を問わず（tagNameは通常大文字だが現在HTMLは普通小文字で書く。齟齬をなくすため大文字小文字不問とする）
        });
        a.html(`main > p > button`, (a)=>{// buttonが対象または親
            a.attr('id', 'my-button-1'); // getAttribute()
        ...
        a.html(`main > p > button`, (a)=>{// buttonが対象または親。セレクタに該当する要素がなければテストエラー
            a.notHas(`セレクタ`); // セレクタに該当する要素は存在しないことを確認する
            // a.hasNotChildren;
            // a.parent.tagNem = 'p';
        });
    });
    a.html((a)=>{// HTMLファイルまたは動的変更結果に対するテスト　documentが対象または親
        // 新しいdocument2で実行する（セレクタがない場合は新しいdocumentを生成する）
        testTarget.appendButtonElement();
    });
})
```
...
* setTimeout等の時間差テスト
* DB応答が必要なテスト（LocalStorage/IndexedDB/RDBMS(SQLite3)）
* ファイルシステム応答が必要なテスト
* サーバ応答が必要なテスト
* ユーザ応答が必要なテスト

https://developer.hatenastaff.com/entry/2016/12/05/102351
...
# js error stacktraceからファイルパス、行数、列数を取得したい ブラウザごとに異なる書式の全パターン

AI による概要
JavaScriptの
Errorオブジェクトのstackプロパティは非標準のプロパティであるため、ブラウザや実行環境によって書式が異なります。特定のブラウザだけでなく、様々な環境に対応するためには、複数のパターンを考慮する必要があります。 
ブラウザごとの一般的な書式パターンと、それらを汎用的にパースするためのアプローチを以下に示します。
ブラウザごとのスタックトレース書式パターン
スタックトレースは通常、エラーメッセージの後に各スタックフレームが改行で区切られて続きます。 

1. Chrome / Edge / Node.js (V8エンジン系)
V8エンジン系の書式は比較的統一されています。関数名、ファイルパス、行数、列数が含まれます。

Error: [エラーメッセージ]
    at [関数名] ([ファイルパス]:[行数]:[列数])
    at [関数名] ([ファイルパス]:[行数]:[列数])
    at Object.([関数名]) ([ファイルパス]:[行数]:[列数])
    ...

    例:

    ReferenceError: invalid is not defined
        at foo (file:///path/to/script.js:10:5)
        at bar (file:///path/to/script.js:15:3)
        at Object.<anonymous> (file:///path/to/script.js:20:1)

2. Firefox (Geckoエンジン系)
Firefoxの書式はChrome系と似ていますが、関数名と位置情報の区切りが異なります。また、@を使用することが特徴です。 
...
...
    ...
    foo@file:///path/to/script.js:10:5
    bar@file:///path/to/script.js:15:3
    @file:///path/to/script.js:20:1

3. Safari (WebKitエンジン系)
Safariの書式は、Chrome系とFirefox系の中間のような形式です。関数名がない場合もあります。
...
...
    ...
    foo@file:///path/to/script.js:10:5
    file:///path/to/script.js:15:3

汎用的なパース方法
すべてのブラウザの書式に確実に対応するには、**error-stack-parser**のような既存のライブラリを使用するのが最も現実的で信頼性の高い方法です。これらのライブラリは、ブラウザ間の書式の違いを抽象化して、構造化されたデータ（関数名、URL、行番号、列番号など）の配列として返してくれます。 
ライブラリを使用した例
javascript
...
try {
    ...
    // stackFramesはStackFrameオブジェクトの配列として取得できる
...
正規表現による自作パーサーの限界
独自の正規表現でパースしようとすると、ブラウザのアップデートや、eval()内のコード、ソースマップの有無、非同期処理によるスタックトレースの変化など、様々なエッジケースに対応しきれなくなる可能性があります。 
可能であれば、StackTrace.jsエコシステムの一部であるerror-stack-parserのような実績のあるツールを利用することを強くお勧めします。これにより、クロスブラウザ互換性の問題を大幅に軽減できます。 
...
　そもそもJavaScript, CSS, HTMLの実装をブラウザだけで行いたい。

　そのためにテキストエディタが欲しい。ファイルシステムに書き込みたい。GitHubにpushしたい。GitHubからcloneしたい。

　Aceなどのブラウザ上で動作するエディタで構文ハイライトしたい。



# 

a.t|真が期待される所で偽になりました。
a.f|偽が期待される所で真になりました。
a.e|例外発生が期待される所で発生しませんでした。／例外の型が違います。／例外のメッセージが違います。／例外の型もメッセージも違います。
a.fn(関数ポインタ, 戻り値)|関数の戻り値が違います。期待値: 実際値:
a.fn(関数ポインタ, [第一引数, 第二引数, ...], 戻り値)|関数の戻り値が違います。期待値: 実際値:
a.fn(関数ポインタ, [[], [第一引数, 第二引数, ...], [第一引数, 第二引数, ...], ...], (...args)=>戻り値)|関数の戻り値が違います。引数idx:N, [第一引数, ] 期待値: 実際値:

    //     ()                                  // 引数なしで呼び出し戻り値が指定した値と一致するかを確認する
